      ...
      <header className="mb-8 border-b-2 border-ink pb-4">
        ...
        <p className="mt-1 font-serif text-sm italic text-ink/70">
          {list.length === 0
            ...
            : `${list.length} carnet${list.length > 1 ? "s" : ""} enregistré${list.length > 1 ? "s" : ""}.`}
        </p>
      </header>
      ...
      {list.length === 0 ? (
        <div className="border-2 border-dashed border-ink/40 p-10 text-center">
          <p className="font-serif italic text-ink/70">
            Les carnets que vous créez en étant connecté apparaîtront ici.
          </p>
          <Link
            href="/fleche"
        ...
        <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {list.map((b) => (
            <li key={b.code}>
              <BookCard
                code={b.code}
                title={b.title?.trim() || "Carnet sans titre"}
